...
CREATE TABLE LINEITEM ( L_ORDERKEY    INTEGER NOT NULL,\
    ...
    con->CreateVectorizedFunction("check_date", {duckdb::LogicalType::DATE}, duckdb::LogicalType::INTEGER, &checkDate);
}

static const int EPOCH_NUM = 3;
int main(int argc, char const *argv[])
{
    ...
    for(int i=0;i<EPOCH_NUM;i++){
        ...
        // auto result1 = con.Query("SELECT o_orderpriority, COUNT(*) AS order_count   FROM orders  WHERE o_orderdate >= (date '1993-7-01')  AND o_orderdate <  (date '1993-10-01')    AND EXISTS (       SELECT *         FROM lineitem        WHERE l_orderkey = o_orderkey AND l_commitdate < l_receiptdate     ) GROUP BY o_orderpriority ORDER BY o_orderpriority");
        auto result1 = con.Query("SELECT o_orderpriority, COUNT(*) AS order_count   FROM orders  WHERE check_date(o_orderdate)   AND EXISTS (       SELECT *         FROM lineitem        WHERE l_orderkey = o_orderkey AND l_commitdate < l_receiptdate     ) GROUP BY o_orderpriority ORDER BY o_orderpriority");
