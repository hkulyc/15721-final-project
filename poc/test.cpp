...
thread_local std::unique_ptr<duckdb::PreparedStatement> prepare = con2.Prepare("SELECT col0+1 from (values ($1))");
...
static void udf_vectorized2(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    auto res = prepare2->Execute(tmp, true);
    if(res->HasError()){
        // std::cout<<res->GetError()<<std::endl;
        return;
    }
    auto data = res->FetchRaw();
